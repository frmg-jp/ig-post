...
OUTPUT_SCHEMA: dict = {
    ...
    "properties": {
        ...
        "story_score": {
            "type": "integer",
            # 2026-09-04 の approval-report で書き直した。「前歴の可視性を
            # 最重要」としていたころ、**足切り(story_min=40)に掛かって0点に
            # なったのに承認された物件が13件あった**（素点は 0/10/10/15×7/
            # 25/35/35）。承認された65件のうち前歴が残っていたのは14%だけで、
            # 「前歴の痕跡は見られない」と書いた物件が何件も承認されている。
            # 最重要としていた条件が、実際には承認の多数派を落としていた。
            "description": (
                "物語の強さを 0〜100 で。様式・年代が特定できること、一点物であること、"
                "設計者が特定できること（特に建築家の自邸）、オリジナルが手つかずで"
                "残っていることを上位に見る。前歴の可視性は**あれば大きな加点**だが、"
                "無いことを理由に下げない。"
            ),
        },
        "story_reason": {
