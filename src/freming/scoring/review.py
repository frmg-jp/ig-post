...
**審査に上がらなかったものは入っていない。** min_to_persist(30) 未満は
一覧に出ないし、人は点数順に並んだ上から見る。ここで測れるのは
「見せたものの中で、人が何を選んだか」であって、「見せなかったものの
中に良いものがあったか」ではない。重みを下げる判断はできるが、
足切りを緩める判断の根拠にはならない。
"""
...
def _suggest(report: ApprovalReport, weights: Mapping[str, float]) -> str:
    ...
    lines.append("")
    lines.append("**この案は「見せたものの中での選ばれ方」しか反映していません。**")
    lines.append("審査に上がらなかった候補（min_to_persist 未満）は入っていないので、")
    lines.append("足切りを緩める・厳しくする判断の根拠にはなりません。")
    return "\n".join(lines)
