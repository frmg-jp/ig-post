...
- summary_en は summary と同じ内容の英語版（1〜2文）。直訳でなくてよいが、
  **日本語版に無い事実を足さない**。
- photo_credit は記事に撮影者名が書いてあるときだけ。**名前だけを返す**
  （"Photography by" のような前置きは含めない）。無ければ空文字。
- Usage / Structure / 面積 / Style も、記事に書かれているものだけ。
  面積は**原文の単位と表記のまま**で、換算しない。
- display_name は投稿の見出しに使う短い英語の物件名。**2〜4語・30字以内。**
  記事に固有名があればそれをそのまま。無ければ年代・様式・種別から
  中立に組む（"1963 Mid-Century Residence" など）。所在地・"in 〜"・
  住所・MLS番号・仲介会社名は入れない。文にしない。**愛称を発明しない。**
- caption_body は投稿の説明文。日本語・です・ます調・3〜6文
  （250〜450字目安）。設計者の文脈→素材・空間構成→履歴や現況の順で、
  **記事の事実だけ**を書く。誇張しない。価格を書かない。
