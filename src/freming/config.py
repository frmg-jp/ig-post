...
class InstagramConfig(BaseModel):
    ...
    # **巡回の下限。** 予定時刻を過ぎているのに出せていないとき（トークン
    # 切れなど）に、0秒で回り続けないための下限。通常はここまで短くならない。
    poll_interval_sec: float = 60.0
    # **巡回の上限。** 投稿ワーカーは次の予定時刻まで寝る。予定が1本も
    # 無くても、たまには起きて見に行く（外から予定を入れられることがある）。
    #
    # ここを短くすると、そのぶんDBを起こす回数が増える。**Neon は接続が
    # 来ている間ずっと計算時間を食う。** 60秒で回し続けた結果、無料枠を
