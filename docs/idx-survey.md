...
- **「住所が漢字だから取れない」は誤り。** 一度そう判断して
  `location_from: none` にしたが、台湾の住所は「縣市＋區」が定型で、
  米国住所とまったく同じように錨にできる。`location_from: tw_address` を
  足して実装し直したところ、**12件中12件で所在地が取れた**。
  og:title → 「地址」の直後 → ページ全体で1種類だけ、の順に見る。
  会社の住所（台北市敦化南路二段267號）は 區 を書かない書式なので
  この式に掛からず、そのまま除外できる。臺 と 台 の表記ゆれは 台 に寄せる
- **URLの結合でパスを1段落とした。** 一覧ページ `/buy` を基点に相対URL
  `house/123` を urljoin すると `/house/123` になり、`/buy` が消える。
  永慶房屋の404を site 側の問題と誤読しかけた


## 2026-08-07 の再確認 — 3件のうち2件を manual_only に降格

自動収集を続けるか判断するため、実際に叩いて確かめた。

| ソース | 物件ページ | 画像 | 判断 |
| --- | --- | --- | --- |
| coldwellbanker | 取れる | **取れる**（1000×666 〜 1280×853） | crawl のまま |
| corcoranicon | 取れる（5,049件） | **1枚も取れない** | manual_only に降格 |
| vanguard | 取れる（9,037件） | **1枚も取れない** | manual_only に降格 |

**corcoranicon と vanguard は同じCDN（`dlajgvw9htjpb.cloudfront.net`）を
使っており、その robots.txt が 403 を返す。** 取得できない robots.txt は
「許可されていない」と扱う方針（fail-closed）なので、画像は全数
スキップされる。物件ページ1件で確認したところ、60枚以上すべてが
スキップされた。画像そのものは 200 で返るが、**方針として取りに行かない。**

画像が無ければ納品も投稿もできない。収集と採点（Claude API）の費用と
審査の手間だけがかかり、成果物は絶対に出ない。CDN の robots.txt が
読めるようになれば crawl に戻せる。

### coldwellbanker を投稿に使うかは別の判断

画像は取れるが**短辺 666〜853px** で、投稿用の 1080×1080 に対して
1.27〜1.62倍に拡大される。眠い絵になるのは承知のうえ、という
2026-08-07 の判断（`images.min_short_edge_px` を 640 に下げた件）と
同じ話がここにも当てはまる。

加えて、**Drive 納品（内部）と Instagram への自動投稿は露出が別物**。
IDX 由来の写真を人のチェックなしで公開することになるので、
`instagram.allowed_sources` で投稿対象から外すかどうかは別途決める。
