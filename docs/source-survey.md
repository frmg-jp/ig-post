...
## 手動URL投入の経路は、いまのままでは納品まで通らない

2026-08-03 に実測して判明。**登録はできるが納品で破綻する。**

```
登録OK  id=1 source=zillow status=approved
納品時に叩きにいったURL: ['https://www.zillow.com/homedetails/12345_zpid']
```

`add_manual_entry` はページを取得しない（設計どおり）。しかし納品時の
`fetch_images` が `source_url` を無条件に GET するため、手動登録した
Zillow の物件を承認すると、納品の瞬間にこちらから Zillow を取りに行く。
**手動投入の前提そのものを壊している。** 実際には robots.txt に弾かれて
`NoImagesFound` になり、`delivery.max_attempts` 回失敗して諦める。

### 足りないのは「人が画像を用意する経路」

必要な実装:

1. 審査UIから画像をアップロードする口（複数枚）
2. アップロードされた画像を `images` テーブルに登録し、
   `source_url` は `manual:<ファイル名>` の形で持つ
3. **手動画像がある物件では `fetch_images` を呼ばない**。
   ここを飛ばさない限り、何を作っても相手サイトを叩く
4. 手動投入なのに画像が無い物件は、承認しても納品キューに入れない
   （失敗を数えても意味がないため）

見積もり: マイグレーション1本、アップロードのエンドポイントとフォーム、
`deliver_property` の分岐、テスト。**半日程度**。
...
|---|---|---|
| 巡回して物件を見つける | 15〜20分 | — |
| 掲載元に画像使用の許諾を取る | 10〜15分 | 数日 |
| フォームに入力 | 3分 | — |
| 画像のダウンロードとアップロード | 5分 | — |
| 審査・納品 | 自動 | — |

許諾が取れないものが出るため、**3〜4件当たって1件成立**と見るのが現実的。
成立1件あたりの実作業は **2時間前後**。

編集メディア経由なら collect → score が自動で回り、審査UIで承認するだけで
**1件あたり数分**。差は20〜30倍ある。

**したがって手動投入は量を稼ぐ経路ではない。** 「この物件をこの企画で出したい」
と決め打ちできる単発に使う。空いているエリアを埋める用途には向かない。
